...
const tokensPath = path.join(__dirname, '../tokens/typography/tokens.json');
...
const TypographyToken = ({ name, token }: { name: string; token: Token }) => {
  const [fontFamily, systemFonts, fontSize, fontWeight, lineHeight, letterSpacing, textTransform, textDecoration] = 
    token.value.split(',').map(item => item.trim());
...
const storiesDir = path.join(__dirname, '../stories/tokens/typography');
if (!fs.existsSync(storiesDir)) {
  fs.mkdirSync(storiesDir, { recursive: true });
}

// Write the story file
const storyPath = path.join(storiesDir, 'TypographyTokens.stories.tsx');
fs.writeFileSync(storyPath, storyTemplate, 'utf8');

// Write the CSS file
const cssPath = path.join(storiesDir, 'TypographyTokens.css');
fs.writeFileSync(cssPath, cssTemplate, 'utf8');

console.log('Generated typography token stories at:', storyPath);
